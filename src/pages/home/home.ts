  ...
  getData(){
    this.sqlite.create({
     ...
     (db:SQLiteObject)=>{
        ...
        db.executeSql('SELECT * FROM exepense ORDER BY rowid DESC')
        .then(res=>{
          this.expenses=[];
        ...
        db.executeSql('SELECT SUM(amount) AS totalIncome FROM expense WHERE type="EXPENSE"')
        .then(res=>{
    ...
    deletData(rowid){
      this.sqlite.create(
        ...
        .then(
          (db:SQLiteObject)=>{
            db.executeSql('DELETE FROM expense WHERE row id=?',[rowid])
              .then(res=>{
                console.log(res);
                this.getData();
              })
